...
impl EvacuateJob {
    ...
    pub fn new(
        ...
        if let Some(of) = obj_file {
            job.evac_type = EvacuateJobType::File(of);
        }
    ...
    pub fn run(mut self) -> Result<(), Error> {
        ...
        let obj_generator_thread = match &job_action.evac_type {
            ...
            EvacuateJobType::File(path) => {
                let channel = crossbeam::bounded(100);
                obj_tx = channel.0;
                obj_rx = channel.1;
                start_file_read(obj_tx,
                    domain.as_str(),
                    Arc::clone(&job_action),
                    path
                )?
...
// Given a single line obtained from the file generated by our sharkspitter,
// parse it in to a SharkspotterMessage which can then be converted to an
// EvacuateObject used by the rebalancer manager.
fn parse_ss_message(s: String) -> Result<SharkspotterMessage, &'static str> {
    let v: Vec<&str> = s.split('\t').collect();
    let shark: String = v[0].to_string();

    let shard: u32 = match v[1].parse::<u32>() {
        Ok(s) => s,
        Err(_) => return Err("Unable to parse shard number.")
    };

    let etag: String = v[2].to_string();

    let obj: Value = match serde_json::from_str(&v[3]) {
        Ok(o) => o,
        Err(_) => return Err("Unable to parse value from object.")
    };

    Ok(SharkspotterMessage {
        manta_value: obj,
        etag,
        shark,
        shard
...
// Instead of reading files from sharkspotter, read them directly from a file
// which contains pre-scanned objects that comprise a storage node that we
// intend to evacuate.  The net result is similar to the sharkspotter
// translation thread except that instead of receiving entries from
// sharkspotter and converting them to EvacuateObjects before passing them
// along to the rebalancer manager, we are reading them from a file.
fn start_file_read(
    obj_tx: crossbeam::Sender<EvacuateObject>,
    domain: &str,
    ...
    let obj_file = path.to_string();

    thread::Builder::new()
        .name(String::from("sharkspitter"))
        .spawn(move || {
            let file = File::open(obj_file)?;
            let reader = BufReader::new(file);

            for line in reader.lines() {
                let l = line.unwrap();
                let ss_msg = match parse_ss_message(l) {
                    Ok(m) => m,
                    Err(e) => {
                        error!("Failed to parse sharkspitter message: {}", e);
                        continue;
                    }
                };
...
mod tests {
    ...
    fn run_full_test(
        ...
        let obj_generator_th = match &job_action.evac_type {
            ...
            _ => start_test_obj_generator_thread(
                obj_tx,
                test_objects,
                job_action.from_shark.manta_storage_id.clone(),
            ),
